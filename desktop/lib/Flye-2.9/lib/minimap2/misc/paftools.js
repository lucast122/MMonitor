...
function paf_asmgene(args) {
    ...
    for (var i = getopt.ind; i < args.length; ++i) {
        ...
        while (file.readline(buf) >= 0) {
            var t = buf.toString().split("\t");
            var ql = parseInt(t[1]), qs = parseInt(t[2]), qe = parseInt(t[3]), mlen = parseInt(t[9]),
                blen = parseInt(t[10]), mapq = parseInt(t[11]);
    ...
    gene_list = gene_list.sort(function (a, b) {
...
function paf_sam2paf(args) {
    ...
    while (file.readline(buf) >= 0) {
        ...
        var clip = [0, 0], soft_clip = 0, I = [0, 0], D = [0, 0], M = 0, N = 0, mm = 0, have_M = false,
            have_ext = false, cigar = [];
