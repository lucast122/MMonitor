...
class Taxonomy:
    ...
    def __init__(self, db):
        ...
        self.app = DjangoDash('taxonomy', external_stylesheets=[dbc.themes.SANDSTONE])
        self._db = db
        
        q = f"SELECT * FROM mmonitor"
        self.df = self._db.query_to_dataframe(q)
    ...
    value=10,
        marks={i: str(i) if i in [1, self.unique_counts // 2, self.unique_counts] else "" for i in range(1, self.unique_counts + 1)},
    ...
    def plot_scatter(self, df):
        ...
        # fig2 = px.scatter(df, x="abundance", y="sample_id", size="sample_id", color="abundance")
    ...
    def _init_callbacks(self) -> None:
        ...
        def plot_selected_figure(value, sample_value_piechart, slider_value) -> Tuple[Figure, Figure, Dict[str, str]]:
            ...
            if value == 'stackedbar':
                ...
                fig1,fig2_style = self.plot_scatter(self.result_df)

            elif value == 'area':
                fig1,fig2_style = self.plot_area(self.result_df)

            elif value == 'scatter3d':
                fig1,fig2 = self.plot_scatter_3d(self.result_df)
